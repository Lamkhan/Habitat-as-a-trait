...
run_models_data_first <- function(dat, i) {
  ...
  m_cs <- update(m_cs_compiled, newdata = dat, chains = 2, cores = 2, iter = 2000, warmup = 1000)
  m_biv <- update(m_biv_compiled, newdata = dat, chains = 2, cores = 2, iter = 2000, warmup = 1000)
...
summarize_causal_models <- function(file_list) {
  ...
  # We'll track runs by their index in the file_list
  for (i in seq_along(file_list)) {
    ...
    fixed_effects$run <- i  # Add run identifier
  ...
  # Create a wide table showing each run's parameter estimates (for fixed effects)
  # This will have one row per run and one column per parameter (for the Estimate column)
  fixed_effects_runs_wide <- all_fixed %>%
    select(run, parameter, Estimate) %>%
    pivot_wider(names_from = parameter, values_from = Estimate)
  
  # Similarly, for random effects:
  random_effects_runs_wide <- all_random %>%
    select(run, parameter, Estimate) %>%
    pivot_wider(names_from = parameter, values_from = Estimate)
  
  # Return the averages and the run-by-run tables
  list(
...
summarize_bivariate_models <- function(file_list) {
  ...
  # Create a wide table for fixed effects by run
  fixed_effects_runs_wide <- all_fixed %>%
    select(run, parameter, Estimate) %>%
    pivot_wider(names_from = parameter, values_from = Estimate)
  
  # Create a wide table for random effects by run
  random_effects_runs_wide <- all_random %>%
    select(run, parameter, Estimate) %>%
    pivot_wider(names_from = parameter, values_from = Estimate)
